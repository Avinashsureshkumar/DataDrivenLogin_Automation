...
Utilities: Includes the Excel data file and other resources.
...
Using pom.xml File:   Navigate to the project directory and run:  mvn clean tes

Using master.xml File:

Execute the tests using TestNG   :   mvn clean test -DsuiteXmlFile=master.xml

Parallel or Cross Browser Execution   :  Configure browsers in the TestNG XML file and run as described above.


#Remote Execution with Selenium Grid:

1)Set execution_env=remote in the info.properties file.

2)Start the Selenium Grid Hub and Nodes.
...
Execute the batch file: ./run-tests.bat
